...
import { OrderItemsService } from 'src/users/services/order-items.service';

@Injectable()
export class CustomerCreateOrderItemsOwnerGuard implements CanActivate {
  constructor(
...
@Injectable()
export class CustomerUpdateOrderItemsOwnerGuard implements CanActivate {
  constructor(
    private reflector: Reflector,
    private usersService: UsersService,
    private ordersService: OrdersService,
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (user.role === 'customer') {
      const idOrderItem = parseInt(request.params?.id || '0', 10);
      const userSign = await this.usersService.findOne(user.sub);
      const orderItem = await this.orderItemsService.findById(idOrderItem);

      if (userSign.customer.id === orderItem.order.customer.id) {
        return true;
      }
    }
